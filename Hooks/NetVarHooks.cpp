...
void __cdecl g_hkSequence( const CRecvProxyData* proxy_data_const, void* entity, void* output )
{
	static auto OriginalFn = Hook->g_phkSequence->GetOriginalFunction();

	C_BasePlayer* local = L::GetLocalPlayer();

	if ( !local || local->GetLifeState() != LifeState::ALIVE )
		return OriginalFn( proxy_data_const, entity, output );
	...
	C_BaseViewModel* view_model = static_cast< C_BaseViewModel* >( entity );

	if ( view_model && view_model->GetOwner() && view_model->GetOwner() != INVALID_EHANDLE_INDEX )
	{
		auto owner = reinterpret_cast<C_BasePlayer*>( g_pClientEntList->GetClientEntityFromHandle( view_model->GetOwner() ) );

		if ( owner == local )
		{
			// Get the filename of the current view model.
			auto knife_model = g_pVModelInfo->GetModel( view_model->GetModelIndex() );
			auto model_name = g_pVModelInfo->GetModelName( knife_model );
	...
	OriginalFn( proxy_data, entity, output );
}
