...
float CMath::GetDelta( float hspeed, float maxspeed, float airaccelerate )
{
...
float CMath::VectorDistance( Vector v1, Vector v2 )
{
	return FASTSQRT( pow( v1.x - v2.x, 2 ) + pow( v1.y - v2.y, 2 ) + pow( v1.z - v2.z, 2 ) );
}

float CMath::GetFov( const QAngle& viewAngle, const QAngle& aimAngle )
{
	Vector ang, aim;
...
float CMath::GetFovB( Vector vLocalOrigin, Vector vPosition, Vector vForward )
{
	Vector vLocal = vPosition - vLocalOrigin;
...
Vector CMath::AnglesToPixels( QAngle SourceAngles, QAngle FinalAngles, float sensitivity )
{
	QAngle delta = SourceAngles - FinalAngles;

	float xMove = ( -delta.x ) / ( 0.022 * sensitivity * 1 );
	float yMove = ( delta.y ) / ( 0.022 * sensitivity * 1 );
