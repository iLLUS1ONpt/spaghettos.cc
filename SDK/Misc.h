...
inline void CrossProduct2(const Vector& vecA, const Vector& vecB, Vector& vecResult)
{
...
// ConVar only
#define FCVAR_PROTECTED			(1<<5)  // It's a server g_pCvar, but we don't send the data since it's a password, etc.  Sends 1 if it's not bland/zero, 0 otherwise as value
#define FCVAR_SPONLY			(1<<6)  // This g_pCvar cannot be changed by clients connected to a multiplayer server.
#define	FCVAR_ARCHIVE			(1<<7)	// set to cause it to be saved to vars.rc
#define	FCVAR_NOTIFY			(1<<8)	// notifies players when changed
#define	FCVAR_USERINFO			(1<<9)	// changes the Client's info string
#define FCVAR_CHEAT				(1<<14) // Only useable in singleplayer / debug / multiplayer & sv_cheats

#define FCVAR_PRINTABLEONLY		(1<<10)  // This g_pCvar's string cannot contain unprintable characters ( e.g., used for player name etc ).
#define FCVAR_UNLOGGED			(1<<11)  // If this is a FCVAR_SERVER, don't log changes to the log file / console if we are creating a log
#define FCVAR_NEVER_AS_STRING	(1<<12)  // never try to print that g_pCvar
...
#define FCVAR_DEMO				(1<<16)  // record this g_pCvar when starting a demo file
#define FCVAR_DONTRECORD		(1<<17)  // don't record these command in demofiles
#define FCVAR_RELOAD_MATERIALS	(1<<20)	// If this g_pCvar changes, it forces a material reload
#define FCVAR_RELOAD_TEXTURES	(1<<21)	// If this g_pCvar changes, if forces a texture reload

#define FCVAR_NOT_CONNECTED		(1<<22)	// g_pCvar cannot be changed by a Client that is connected to a server
#define FCVAR_MATERIAL_SYSTEM_THREAD (1<<23)	// Indicates this g_pCvar is read from the material system thread
#define FCVAR_ARCHIVE_XBOX		(1<<24) // g_pCvar written to config.cfg on the Xbox

#define FCVAR_ACCESSIBLE_FROM_THREADS	(1<<25)	// used as a debugging tool necessary to check material system thread convars

#define FCVAR_SERVER_CAN_EXECUTE	(1<<28)// the server is allowed to execute this command on clients via ClientCommand/NET_StringCmd/CBaseClientState::ProcessStringCmd.
#define FCVAR_SERVER_CANNOT_QUERY	(1<<29)// If this is set, then the server is not allowed to query this g_pCvar's value (via IServerPluginHelpers::StartQueryCvarValue).
